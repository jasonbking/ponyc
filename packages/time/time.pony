...
use @clock_gettime[I32](clock: U32, ts: Pointer[(I64, I64)])
  if lp64 and (linux or bsd or illumos)

use @clock_gettime[I32](clock: U32, ts: Pointer[(I32, I32)])
  if ilp32 and (linux or bsd or illumos)
...
primitive Time
  ...
  fun now(): (I64 /*sec*/, I64 /*nsec*/) =>
    ...
    ifdef osx then
      ...
      (ts._1, ts._2 * 1000)
    elseif linux or bsd or illumos then
  ...
  fun _clock_gettime(clock: _Clock): (I64, I64) =>
    ...
    """
    ifdef lp64 and (linux or bsd or illumos) then
      var ts: (I64, I64) = (0, 0)
      @clock_gettime(clock(), addressof ts)
      ts
    elseif ilp32 and (linux or bsd or illumos) then
      var ts: (I32, I32) = (0, 0)
      @clock_gettime(clock(), addressof ts)
      (ts._1.i64(), ts._2.i64())
    else
