...
primitive Sig
  ...
  fun ill(): U32 =>
    ifdef linux or bsd or osx or illumos then 4
    else compile_error "no SIGINT"
    end

  fun trap(): U32 =>
    ifdef linux or bsd or osx or illumos then 5
  ...
  fun emt(): U32 =>
    ifdef bsd or osx or illumos then 7
    else compile_error "no SIGEMT"
    end

  fun fpe(): U32 =>
    ifdef linux or bsd or osx or illumos then 8
  ...
  fun bus(): U32 =>
    ifdef bsd or osx or illumos then 10
    elseif linux then 7
    else compile_error "no SIGBUS"
    end

  fun segv(): U32 =>
    ifdef linux or bsd or osx or illumos then 11
    else compile_error "no SIGSEGV"
    end

  fun sys(): U32 =>
    ifdef bsd or osx or illumos then 12
    elseif linux then 31
    else compile_error "no SIGSYS"
    end

  fun pipe(): U32 =>
    ifdef linux or bsd or osx or illumos then 13
  ...
  fun usr2(): U32 =>
    ifdef not "scheduler_scaling_pthreads" then
      ...
      elseif illumos then 17
      else compile_error "no SIGUSR2"
      end
    else
      ifdef linux or bsd or osx or illumos then
