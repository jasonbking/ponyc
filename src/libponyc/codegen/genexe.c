...
#if defined(PLATFORM_IS_LINUX) || defined(PLATFORM_IS_BSD) || defined(PLATFORM_IS_ILLUMOS)
  ...
  const char* file_o)
{
...
#elif defined(PLATFORM_IS_LINUX) || defined(PLATFORM_IS_BSD) || defined(PLATFORM_IS_ILLUMOS)
  const char* file_exe =
    suffix_filename(c, c->opt->output, "", c->filename, "");
  ...
  const char* avl = target_is_illumos(c->opt->triple) ? "-lavl" : "";
...
// The use of NDEBUG instead of PONY_NDEBUG here is intentional.
    ...
    lib_args, dtrace_args, ponyrt, ldl, lexecinfo, nsl, avl
    );
